...
// wrapc file define:
#define AUTOTB_TVIN_gmem1 "../tv/cdatafile/c.workload.autotvin_gmem1.dat"
#define AUTOTB_TVOUT_gmem1 "../tv/cdatafile/c.workload.autotvout_gmem1.dat"
// wrapc file define:
#define AUTOTB_TVIN_gmem2 "../tv/cdatafile/c.workload.autotvin_gmem2.dat"
#define AUTOTB_TVOUT_gmem2 "../tv/cdatafile/c.workload.autotvout_gmem2.dat"
// wrapc file define:
...
// tvout file define:
#define AUTOTB_TVOUT_PC_gmem1 "../tv/rtldatafile/rtl.workload.autotvout_gmem1.dat"
// tvout file define:
#define AUTOTB_TVOUT_PC_gmem2 "../tv/rtldatafile/rtl.workload.autotvout_gmem2.dat"
// tvout file define:
    ...
    string AESL_num;{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<512> > gmem_pc_buffer(16384);
          int i = 0;
...
((long long*)__xlx_apatb_param_result)[j*8+7] = gmem_pc_buffer[i].range(511,448).to_int64();
    ...
    } // end post check logic bolck
  {
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<512> > gmem1_pc_buffer(16384);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 16384; j < e; j += 1, ++i) {((long long*)__xlx_apatb_param_temp)[j*8+0] = gmem1_pc_buffer[i].range(63,0).to_int64();
((long long*)__xlx_apatb_param_temp)[j*8+1] = gmem1_pc_buffer[i].range(127,64).to_int64();
((long long*)__xlx_apatb_param_temp)[j*8+2] = gmem1_pc_buffer[i].range(191,128).to_int64();
((long long*)__xlx_apatb_param_temp)[j*8+3] = gmem1_pc_buffer[i].range(255,192).to_int64();
((long long*)__xlx_apatb_param_temp)[j*8+4] = gmem1_pc_buffer[i].range(319,256).to_int64();
((long long*)__xlx_apatb_param_temp)[j*8+5] = gmem1_pc_buffer[i].range(383,320).to_int64();
((long long*)__xlx_apatb_param_temp)[j*8+6] = gmem1_pc_buffer[i].range(447,384).to_int64();
((long long*)__xlx_apatb_param_temp)[j*8+7] = gmem1_pc_buffer[i].range(511,448).to_int64();
}}}
        } // end transaction
      } // end file is good
    } // end post check logic bolck
...
__xlx_tmp_lv.range(511,448) = ((long long*)__xlx_apatb_param_temp)[j*8+7];
  ...
  tcl_file.set_num(16384, &tcl_file.gmem1_depth);
  sprintf(__xlx_sprintf_buffer.data(), "[[/transaction]] \n");
  aesl_fh.write(AUTOTB_TVIN_gmem1, __xlx_sprintf_buffer.data());
}
